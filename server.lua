...
AddEventHandler('esx_peds:buyItem', function(item, price)
    ...
    if xPlayer.getInventoryItem(item).limit == -1 then
        xPlayer.removeMoney(price)
        xPlayer.addInventoryItem(item, 1)
        TriggerClientEvent('esx:showNotification', source, "Vous venez d'acheter un ~g~"..xPlayer.getInventoryItem(item).label)
    elseif xPlayer.getInventoryItem(item).count + 1 > xPlayer.getInventoryItem(item).limit then TriggerClientEvent('esx:showNotification', source, "Vous n'avez plus assez de place dans votre ~r~inventaire") return end
    xPlayer.removeMoney(price)
    xPlayer.addInventoryItem(item, 1)
    TriggerClientEvent('esx:showNotification', source, "Vous venez d'acheter un ~g~"..xPlayer.getInventoryItem(item).label)
end)
